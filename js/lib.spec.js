...
//     expect(getElement1(arr, 0)).to.equal('a');
//     expect(getElement1(arr, 2)).to.equal('c');
//     expect(getElement1(arr, 4)).to.equal('e');
//   });

//   it('підтримує негативні індекси (від кінця масиву)', () => {
//     expect(getElement1(arr, -1)).to.equal('e');
//     expect(getElement1(arr, -3)).to.equal('c');
//     expect(getElement1(arr, -5)).to.equal('a');
//   });

//   it('повертає fallback, коли індекс виходить за межі масиву', () => {
//     expect(getElement1(arr, 10)).to.be.null;                 // за праву межу
//     expect(getElement1(arr, -10)).to.be.null;                // за ліву межу
//     expect(getElement1(arr, 5, 'oops')).to.equal('oops');
//     expect(getElement1(arr, -6, 123)).to.equal(123);
//   });

//   it('повертає fallback за замовчуванням (null), якщо не передано третій параметр', () => {
//     expect(getElement1(arr, 999)).to.be.null;
//     expect(getElement1(arr, -999)).to.be.null;
//   });
...
//       expect(() => getElement1(value, 0)).to.throw(TypeError, /масивом/);
//     });
//   });
...
//       expect(() => getElement1(arr, idx)).to.throw(TypeError, /числом/);
//     });

//     // Але нормальні числа (включаючи +0, -0, Infinity) — дозволені
//     expect(() => getElement1(arr, Infinity)).to.not.throw();
//     expect(getElement1(arr, Infinity)).to.be.null;
//     expect(getElement1(arr, -Infinity)).to.be.null;
//   });

//   it('коректно працює з порожнім масивом', () => {
//     const empty = [];
//     expect(getElement1(empty, 0)).to.be.null;
//     expect(getElement1(empty, -1)).to.be.null;
//     expect(getElement1(empty, 0, 'fallback')).to.equal('fallback');
//   });

//   it('повертає undefined, NaN, false тощо, якщо вони є в масиві (не плутає з fallback)', () => {
//     const weird = [undefined, null, false, 0, '', NaN];
//     expect(getElement1(weird, 0)).to.be.undefined;
//     expect(getElement1(weird, 1)).to.be.null;
//     expect(getElement1(weird, 2)).to.be.false;
//     expect(getElement1(weird, 5)).to.be.NaN;
//   });

//   // Додатковий крайній випадок — дуже великі/малі індекси
//   it('працює з дуже великими або дуже малими індексами', () => {
//     expect(getElement1(arr, 1e10)).to.be.null;
//     expect(getElement1(arr, -1e10)).to.be.null;
//     expect(getElement1(arr, Number.MAX_SAFE_INTEGER)).to.be.null;
//   });
// });
// });
// });  
